...
_VIDEO_HREF_RE = re.compile(
    r"(?:txxx\.com|txxx\.tube)/(?:videos?|embed)/(?P<id>\d+)(?:/(?P<slug>[^/?#]+))?/?",
    re.IGNORECASE,
)
...
def can_handle(host: str) -> bool:
    ...
    if h in SITE_ALIASES:
        return True
    return h.endswith(".txxx.com") or h.endswith(".txxx.tube")
...
def _normalize_site_host(host: str) -> str:
    ...
    if h.startswith("www."):
        h = h[4:]
    if h in SITE_ALIASES or h.endswith(".txxx.com") or h.endswith(".txxx.tube"):
        return h
    return SITE_HOST
...
def _parse_list_context(base_url: str) -> dict[str, Any]:
    ...
    parsed = urlparse(raw if raw.startswith("http") else f"https://txxx.com{raw if raw.startswith('/') else '/' + raw}")
    host = _normalize_site_host(parsed.netloc or SITE_HOST)
...
def _preview_url_from_value(preview: Any, *, host: str) -> Optional[str]:
    if not preview:
        return None
    url = _normalize_media_url(str(preview), host=host)
    if not url:
        return None
    ...
    if url.startswith("//"):
        return f"https:{url}"
    return f"https://{url.lstrip('/')}" if "." in url.split("/")[0] else _normalize_media_url(f"/{url.lstrip('/')}", host=host)
...
def _list_item_from_video_row(row: dict[str, Any], *, host: str) -> dict[str, Any]:
    ...
    return {
        ...
        "preview_url": _preview_url_from_value(row.get("pv"), host=host),
    }
...
def parse_video_info(data: dict[str, Any], *, host: str, video_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "related_videos": [],
        "preview_url": _preview_url_from_value(video.get("pv"), host=host),
    }
...
async def scrape(url: str) -> dict[str, Any]:
    ...
    if not video_id:
        raise ValueError("Could not extract TXXX video id from URL")
