...
BASE_SITE = "https://hotmovs.tube/"
SITE_HOST = "hotmovs.tube"
...
_VIDEO_HREF_RE = re.compile(
    r"(?:hotmovs\.tube|hotmovs\.com)/(?:videos?|embed)/(?P<id>\d+)(?:/(?P<slug>[^/?#]+))?/?",
    re.IGNORECASE,
)
...
def can_handle(host: str) -> bool:
    ...
    if h in SITE_ALIASES:
        return True
    return h.endswith(".hotmovs.tube") or h.endswith(".hotmovs.com")
...
def _normalize_site_host(host: str) -> str:
    ...
    if h.startswith("www."):
        h = h[4:]
    if h in SITE_ALIASES or h.endswith(".hotmovs.tube") or h.endswith(".hotmovs.com"):
        return h
    return SITE_HOST
...
def _parse_list_context(base_url: str) -> dict[str, Any]:
    ...
    parsed = urlparse(raw if raw.startswith("http") else f"https://hotmovs.tube{raw if raw.startswith('/') else '/' + raw}")
    host = _normalize_site_host(parsed.netloc or SITE_HOST)
...
async def scrape(url: str) -> dict[str, Any]:
    ...
    if not video_id:
        raise ValueError("Could not extract HotMovs video id from URL")
