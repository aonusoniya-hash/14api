...
_MD5_SALT = "9826avrbi6m49vd7shxkn9815"
...
def _clean_title(title: str | None) -> Optional[str]:
    ...
    if t.lower() in {"loading...", "loading", "please wait..."}:
        return None
    for suffix in (
        " | JavFun",
        " - JavFun",
...
def _site_origin(url: str) -> str:
    ...
    host = parsed.netloc or "en.javfun.me"
...
def _normalize_video_href(href: str, *, base: str = BASE_SITE) -> Optional[str]:
    ...
    lower = raw.lower()
    if "/movies/" not in lower:
        return None
    if "javfun.me" not in lower:
        return None
    parsed = urlparse(raw)
    ...
    if len(parts) < 2 or parts[0] != "movies":
        return None
    slug = parts[1]
    host = parsed.netloc or urlparse(base).netloc or "en.javfun.me"
    return f"{parsed.scheme or 'https'}://{host}/movies/{slug}"


def _parse_episode_id(html: str) -> Optional[str]:
    match = _MOVIE_OBJECT_RE.search(html or "")
    if match:
        return match.group("id")
    match = _EPISODE_ID_RE.search(html or "")
    if match:
        return match.group("id")
    hidden = re.search(
...
def _parse_playlist_sources(data: dict[str, Any], *, base: str) -> list[dict[str, Any]]:
    ...
    for item in playlist:
        ...
        for source in sources:
            ...
            if not file_url:
                continue
            file_url = _normalize_media_url(str(file_url), base=base)
...
async def _fetch_sources_json(episode_id: str, *, referer: str) -> dict[str, Any]:
    ...
    api_url = f"{origin}ajax/get_sources/{episode_id}/{token}?count=1&mobile=0"
...
async def _extract_video_streams(html: str, *, page_url: str) -> dict[str, Any]:
    ...
    episode_id = _parse_episode_id(html)
    streams: list[dict[str, Any]] = []

    if episode_id:
        data = await _fetch_sources_json(episode_id, referer=page_url)
        streams = _parse_playlist_sources(data, base=origin)
...
def parse_video_page(html: str, url: str) -> dict[str, Any]:
    ...
    title_el = soup.select_one("#mv-info h3, .main-detail h3, .page-detail h3, h1")
    title = _clean_title(
        _first_non_empty(
    ...
    studio_link = soup.select_one(
        ".breadcrumb a[href*='/studio/'], #mv-info a[href*='/studio/'], .mv-info a[href*='/studio/']"
    )
    uploader = _first_non_empty(
        studio_link.get_text(" ", strip=True) if studio_link else None,
        *(
            a.get_text(" ", strip=True)
            for a in soup.select("#mv-info a[href*='/pornstar/'], .mv-info a[href*='/pornstar/']")[:2]
        ),
    )

    category_link = soup.select_one("a[href*='/category/']")
    category = _first_non_empty(
        category_link.get_text(" ", strip=True) if category_link else None,
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    if not parts:
        ...
        return urlunparse((scheme, host, f"/japanese-porn-videos/page-{page_num}", "", "", ""))

    if parts[0] == "japanese-porn-videos" and len(parts) == 1:
        if page_num <= 1:
            return urlunparse((scheme, host, "/japanese-porn-videos/", "", "", ""))
        return urlunparse((scheme, host, f"/japanese-porn-videos/page-{page_num}", "", "", ""))

    if parts[0] in ("search", "category", "studio", "tag", "pornstar") and len(parts) >= 2:
        slug = "/".join(parts[1:])
        if page_num <= 1:
            path = f"/{parts[0]}/{slug}/"
        else:
    ...
    return urlunparse((scheme, host, "/" + "/".join(parts) + f"/page-{page_num}", "", "", ""))
...
def _parse_list_item(box: Any, *, base: str) -> Optional[dict[str, Any]]:
    link = box.select_one("a.ml-mask[href*='/movies/'], a[href*='/movies/']")
    if not link:
        return None
...
async def list_videos(base_url: str, page: int = 1, limit: int = 100) -> list[dict[str, Any]]:
    ...
    for box in soup.select("div.ml-item[data-movie-id], div.ml-item"):
        if len(items) >= limit:
            break
    ...
    if not items:
        for link in soup.select("a.ml-mask[href*='/movies/'], a[href*='/movies/']"):
            if len(items) >= limit:
                break
