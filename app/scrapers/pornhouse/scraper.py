...
BASE_SITE = "https://pornhouse.me/"
SITE_HOST = "pornhouse.me"
SITE_ALIASES = frozenset({"pornhouse.me", "www.pornhouse.me", "cdn.pornhouse.me"})
...
def can_handle(host: str) -> bool:
    ...
    if h in SITE_ALIASES:
        return True
    return h.endswith(".pornhouse.me")
...
def _site_origin(url: str) -> str:
    ...
    host = parsed.netloc or SITE_HOST
...
def _normalize_video_href(href: str, *, base: str = BASE_SITE) -> Optional[str]:
    ...
    if len(parts) < 2 or parts[0] not in ("movies", "movie"):
        return None
    slug = parts[1]
    host = parsed.netloc or urlparse(base).netloc or SITE_HOST
...
def parse_video_page(html: str, url: str) -> dict[str, Any]:
    ...
    for link in soup.select(
        ".main-detail a[href*='/category/'], .mv-info a[href*='/category/'], a[href*='/tag/']"
    ):
    ...
    uploader = _first_non_empty(
        ...
        *(
            ...
            for a in soup.select(
                ".main-detail a[href*='/actor/'], .mv-info a[href*='/actor/'], "
                "#mv-info a[href*='/pornstar/'], .mv-info a[href*='/pornstar/']"
            )[:3]
        ),
    )
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    if parts[0] in ("category", "studio", "tag", "pornstar") and len(parts) >= 2:
        slug = "/".join(parts[1:])
        if page_num <= 1:
            path = f"/{parts[0]}/{slug}/"
        else:
...
async def list_videos(base_url: str, page: int = 1, limit: int = 100) -> list[dict[str, Any]]:
    ...
    for box in soup.select("div.ml-item[data-movie-id], div.ml-item.item, div.ml-item, .item"):
        if len(items) >= limit:
            break
